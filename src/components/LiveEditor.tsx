...
import { Mic, Play, Square, Sparkles, Zap } from 'lucide-react';
...
export const LiveEditor = ({ content, onContentChange, isLive, onToggleLive, selectedTemplate }: LiveEditorProps) => {
  const editorRef = useRef<HTMLTextAreaElement>(null);
  const { generateText, isGenerating, currentTask } = useLocalAI();
  ...
  // Auto-generate content when live mode is active
  useEffect(() => {
    if (isLive && content.length > 10 && content !== lastPrompt && !isGenerating) {
      const timer = setTimeout(async () => {
        const lastSentence = content.split('.').pop()?.trim();
        if (lastSentence && lastSentence.length > 5) {
          try {
            const generatedContent = await generateText(lastSentence, selectedTemplate);
  ...
  }, [content, isLive, generateText, selectedTemplate, lastPrompt, isGenerating, onContentChange]);
  ...
  const handleManualGenerate = async () => {
    if (content.trim()) {
      try {
        const generated = await generateText(content, selectedTemplate);
        onContentChange(content + '\n\n' + generated);
      } catch (error) {
        console.error('Error generating content:', error);
      }
        ...
        <div className="flex items-center gap-4">
          ...
          {(isLive || isGenerating) && (
            <div className="text-xs text-green-400 px-3 py-1 bg-green-500/20 rounded-full border border-green-500/30">
              ...
              {currentTask || 'جاري التأليف الذكي'}
            </div>
          )}
        </div>
          ...
          <div className="absolute bottom-6 right-6">
            <div className="flex items-center gap-3 px-4 py-2 bg-gray-800/90 backdrop-blur-sm rounded-2xl border border-gray-700 shadow-xl">
              ...
              <span className="text-sm font-medium text-green-400">
                {currentTask || 'الذكاء الاصطناعي يعمل'}
              </span>
            </div>
          </div>
        )}
